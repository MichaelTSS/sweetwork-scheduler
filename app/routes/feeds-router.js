...
router.post(
      ...
      cli.hget({ key, field: 'id' }).then(
        id => {
          if (id === null) {
            ...
            res.status(500).json({ success: false, error, where: 'check-if-feed-exists' });
          } else {
            next();
          }
        },
        e => {
          res.status(500).json({ success: false, error: e, where: 'check-if-feed-exists' });
        }
      );
    } catch (e) {
      logger.error(e);
    ...
    try {
      if (req.body.error) {
        ...
        if (req.body.error.name === 'Warning') {
          logger.error(`Api Service recorded a warning ${JSON.stringify(req.body.error)}`);
          cli
            .zadd({
              key: RedisKeys.feedWarningTicks(req.body.error.clientId, req.body.source),
              scomembers: [String(moment().unix()), JSON.stringify(req.body.error)]
            })
            .catch(e => logger.error(`Warning ticks ${e}`));
        } else if (req.body.error.name === 'Error') {
          logger.error(`Api Service recorded an error ${JSON.stringify(req.body.error)}`);
          cli
            .zadd({
              key: RedisKeys.feedErrorTicks(req.body.error.clientId, req.body.source),
              scomembers: [String(moment().unix()), JSON.stringify(req.body.error)]
            })
      ...
      logger.error(e);
      res.status(500).json({ success: false, error: e, where: 'create-error-plot-lines' });
    }
  },
  (req, res, next) => {
    try {
      ...
      cli.zcount({ key }).then(count => {
        if (count === 0) {
          cli.zadd({ key, scomembers: [req.body.timestamp_from, '0'] }).catch(e => logger.error(`First error band ${e}`));
        }
      });
      if (req.body.error) {
        // the error
        ...
        if (req.body.ticks.length > 0) {
          score = Math.round(parseInt(req.body.ticks[req.body.ticks.length - 1], 10) / 1000);
          logger.info('Making/updating a hole because got some data but errored');
        } else {
        ...
        try {
          cli.zrangebyscore({ key, min: req.body.timestamp_from, max: req.body.timestamp_to, withscores: true }).then(members => {
            if (members.length > 0) {
              logger.info(
                `Updating a hole. Was from ${moment.unix(members[0]).fromNow()}` +
                  ` to ${moment.unix(members[1]).fromNow()}, now to ${moment.unix(score).fromNow()}`
              );
              cli.zadd({ key, scomembers: [score, members[0]] }).catch(e => logger.error(`update hole ${e}`));
            } else {
              logger.info(`New hole is from ${moment.unix(req.body.timestamp_from).fromNow()}` + ` to ${moment.unix(score).fromNow()}`);
              cli.zadd({ key, scomembers: [score, String(req.body.timestamp_from)] }).catch(e => logger.error(`create hole ${e}`));
            }
          });
        } catch (e) {
          logger.error(e);
          res.status(500).json({ success: false, error: e, where: 'update-error-plot-bands' });
        }
      } else {
        cli.zrangebyscore({ key, min: req.body.timestamp_from, max: req.body.timestamp_to }).then(members => {
          if (members.length > 0) {
      ...
      logger.error(e);
      res.status(500).json({ success: false, error: e, where: 'create-error-plot-bands' });
    }
  },
  (req, res, next) => {
    try {
      if (req.body.ticks && req.body.ticks.length > 0) {
        ...
        const scomembers = _.flatMap(req.body.ticks, n => [Math.round(parseInt(n, 10) / 1000), String(n)]);
        //
        cli.zadd({ key: RedisKeys.feedTicks(req.body.id, req.body.source), scomembers }).then(
          count => {
        ...
        cli.zadd({ key: RedisKeys.feedEfficiencyTicks(req.body.id, req.body.source), scomembers: scomembersEfficiency }).then(
          count => {
  ...
  (req, res, next) => {
    try {
      ...
      req.body.density = utils.computeDensity(req.body.num_results, req.body.timestamp_from, req.body.timestamp_to);
      ...
      logger.error(e);
      res.status(500).json({ success: false, error: e, where: 'update-feed-hash' });
    }
  },
  (req, res, next) => {
    try {
      ...
      if (
        ...
        nextTickCrawl = moment()
          .add(1, 'minutes')
          .unix();
      } else if (!req.body.density && req.body.error && req.body.error.message === 'Not supported') {
        nextTickCrawl = moment()
          .add(1, 'hours')
          .unix();
      } else {
        const sourceSettings = SOURCE_SETTINGS[req.body.source][req.body.entity];
        if (sourceSettings.page_size !== null) {
          let intervalNeededToFillPagination = sourceSettings.page_size * 60 * 60 / req.body.density;
          if (sourceSettings.is_pagination === false) {
            // since we cannot paginate, we want to fetch a bit earlier
            intervalNeededToFillPagination = Math.round(intervalNeededToFillPagination * 0.9);
          }
          const tmp = Math.min(SOURCE_SETTINGS[req.body.source].max_interval_ticks, intervalNeededToFillPagination);
          nextTickCrawl = Math.round(unixNow + Math.max(SOURCE_SETTINGS[req.body.source].min_interval_ticks, tmp));
        } else {
          nextTickCrawl = unixNow + SOURCE_SETTINGS[req.body.source].default_interval_ticks;
        }
        // Store the next tick for this feed to be fetched
        logger.info(
          `Fetched ${req.body.num_results} posts for feed (${req.body.source}:${req.body.id})` +
            ` of density = ${req.body.density} per hour.`
        );
      }
      logger.info(`Next crawl set ${moment.unix(nextTickCrawl).fromNow()}`);
      cli
        .zadd({ key: RedisKeys.feedsList(), scomembers: [nextTickCrawl, feedKey] })
        .catch(e => logger.error(`Next crawl ${JSON.stringify(e)}`));
      res.status(200).json({ success: true });
    } catch (e) {
      logger.error(e);
      res.status(500).json({ success: false, error: e, where: 'create-next-ticks' });
    }
  }
);
