...
controller.auth('Scheduler').catch(logger.error); // RPC call to crawler
        ...
        member => {
          cli.hgetall({ key: member }).then(
            feedHash => {
              ...
              const nextCrawlDuration = feedHash.last_time_crawl
                ? NEXT_CRAWL_DURATION
                : 3 * NEXT_CRAWL_DURATION;
              const score = moment().unix() + 60 * nextCrawlDuration;
              //
              if (feedHash.status === 'busy') {
                logger.warn(
                  'This feed has not been updated since it was first ordered to crawl',
                );
              } else {
                // update that feedHash to be "busy"
                cli
                  .hset({ key: member, field: 'status', value: 'busy' })
                  .catch(logger.error);
              }
              //
              cli
                ...
                .then(
                  keys => {
                    ...
                    Promise.all(topicsListPromises).then(topicHashes => {
                      feedToSearch.topic_hash = _.transform(
                        topicHashes,
                        (result, hash) => {
                          (result[hash.client_id] ||
                            (result[hash.client_id] = [])
                          ).push(hash.id);
                        },
                        {},
                      );
                      feedToSearch.languages = _.uniq(
                        _.flattenDeep(
                          topicHashes.map(t =>
                            t.languages.split(',').filter(f => !!f),
                          ),
                        ),
                      );
                      feedToSearch.countries = _.uniq(
                        _.flattenDeep(
                          topicHashes.map(t =>
                            t.countries.split(',').filter(f => !!f),
                          ),
                        ),
                      );
                      setTimeout(() => {
                        logger.info(
                          `Trigger search for ${feedToSearch.source}:${feedToSearch.id} up until ` +
                            `${moment().to(
                              moment.unix(feedToSearch.timestamp_from),
                            )}`,
                        );
                        controller.search(feedToSearch).catch(err => {
                          logger.error(err);
                              ...
                              if (mbers.length > 0) {
                                logger.info(
                                  `Updating a hole. Was from ${moment
                                    ...
                                    ` to ${moment
                                      .unix(mbers[1])
                                      .fromNow()}, now to ${moment
                                      .unix(errTo)
                                      .fromNow()}`,
                                );
                                cli
                                  .zadd({ key, scomembers: [errTo, mbers[0]] })
                                  .catch(e => logger.error(`update hole ${e}`));
                              } else {
                                logger.info(
                                  `New hole is from ${moment
                                    .unix(feedToSearch.timestamp_from)
                                    .fromNow()}` +
                                    ` to ${errTo}, ${moment
                                      .unix(errTo)
                                      .fromNow()}`,
                                ...
                                cli
                                  .zadd({
                                    ...
                                    scomembers: [
                                      errTo,
                                      String(feedToSearch.timestamp_from),
                                    ],
                                  })
              ...
              cli
                .zadd({ key: feedsListKey, scomembers: [score, member] })
                .catch(logger.error);
            },
            e => logger.error(e),
          );
